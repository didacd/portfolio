...
> This is regular text. Whatever you put here will be rendered normally.
> You can also add **bold**, *italic*, and other text modifications. [Links too!](https://example.com).
> `You can even put single code lines, like on GitHub!`
...
> [!note] Keep site-specific profile settings, such as your social links and GitHub username, in `src/config/site.yaml`.
...
The `config/site.yaml` file holds profile settings, including the social links in the second card on the home page.

```yaml title="src/config/site.yaml"
profile:
  socials:
    - id: 1
      icon:
        type: lucide # lucide or simple-icons
        name: github
      text: GitHub
